...
export default {
    async init() {
        ...
        s.onload = () => {
            stripe = Stripe('pk_test_51Kok8YGZLyNaXlTGp3nIkGM1m0WwcMixvVq2uNp7I0mnKAEm7SQqLV8Q7p75uZhQmvYlJvmMZsvN8oITXFAeQ7vc00fk2DVZPR');
        };
        s.defer = true;
    ...
    async onCheckout() {
        try {
        const url = new URL("https://hugosamplesite.netlify.app/checkout");
    ...
    render() {
        ...
        const info = cart.map(x => ({...x, price: parseFloat(products[x.name].Price), 
            cover: products[x.name].Cover}));

        if (itemList) {
            itemList.innerHTML = info.map(x =>
                `<div class="item">
